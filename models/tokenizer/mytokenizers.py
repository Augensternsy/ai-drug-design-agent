...
class regexTokenizer():
    ...
    def encode_one(self, smi):
        """
         ...
         """
        res = []  # 初始化 res
        for i in self.rg.findall(smi):
            if i in self.toktoid:
                res.append(self.toktoid[i])
            else:
                # 使用未知标记 [UNK]，不动态添加新token以避免索引越界
                # 使用最大的有效ID作为UNK token（最后一个vocab token）
                unk_id = len(self.idtotok) - 1
                res.append(unk_id)

        # 开头添加 [SOS],末尾添加 [EOS]
        res = [1] + res + [2]
        if len(res) < self.max_len:
            # 如果 res 的长度小于最大长度 self.max_len，则在末尾填充 [PAD]
            res += [0] * (self.max_len - len(res))
        else:
            #  res 的长度大于或等于 max_len，则截取 res 的前 max_len 个元素，并将最后一个元素设为 [EOS]
    ...
    def corrupt_one(self, smi):
        ...
        for i in res:
            if i is not None:
                if i in self.toktoid:
                    res_ids.append(self.toktoid[i])
                else:
                    # 使用UNK token
                    unk_id = len(self.idtotok) - 1
                    res_ids.append(unk_id)
        res = [1] + res_ids + [2]
        if len(res) < self.max_len:
            res += [0] * (self.max_len - len(res))
        else:
...
class ChemformerTokenizer():
    def __init__(self, max_len=170):
        print('ChemformerTokenizer Truncating length:', max_len)
        # Use the vocabulary from tokenizer_smiles_diffusion.json but adapted for our format:
        # 0: '[PAD]', 1: '[SOS]', 2: '[EOS]', 3: '[UNK]'
        # 4+: the chemical tokens
        chem_tokens = [
            "O", "=", "C", "1", "N", "(", "/", "c", "2", "n", "[nH]", ")", 
            "[C@H]", "[C@@H]", "3", "Br", "F", "S", "Cl", "\\", "[N+]", "[O-]", 
            "#", "4", "s", "-", "o", "5", "[C@@]", "[C@]", "6", "P", "7"
        ]
        self.idtotok = {
            0: '[PAD]',
        ...
        for idx, tok in enumerate(chem_tokens):
            self.idtotok[idx + 4] = tok
            
        self.toktoid = {v: k for k, v in self.idtotok.items()}
        self.vocab_size = len(self.idtotok)
        self.max_len = max_len
        
        # Use the exact same regex pattern to tokenize SMILES
        pattern = "(\\[[^\\]]+]|Br?|Cl?|N|O|S|P|F|I|b|c|n|o|s|p|\\(|\\)|\\.|=|#|-|\\+|\\\\|\\/|:|~|@|\\?|>|\\*|\\$|\\%[0-9]{2}|[0-9])"
        self.rg = regex.compile(pattern)
        self.len = len(self.idtotok)
