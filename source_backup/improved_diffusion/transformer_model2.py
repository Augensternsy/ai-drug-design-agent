...
class TransformerNetModel2(nn.Module):
    ...
    def forward(self, x, timesteps, desc_state, desc_mask ,y=None, src_ids=None, src_mask=None):
        ...
        #################################################################
        # print(f"desc_mask 形状: {desc_mask.shape}")
        # desc_state = desc_state.unsqueeze(-1)
        ...
        # 否则 BertSelfAttention 的 cross-attention 会错误地将 0 视作有源信号，导致注意力被无意义的 Padding 稀释
        ...
        # print(f"处理后 desc_state 形状: {desc_state.shape}")  # 目标输出：[4, 1, 1, 216]（假设 seq_len_k=216）
        # print(f"处理后 desc_state 维度数: {desc_state.dim()}")
        # print(f"desc_state 形状: {desc_state.shape}")
        ...
        # print(f"DEBUG - emb_x shape: {emb_x.shape}")
        # print(f"DEBUG - emb shape: {emb.shape}")
        # print(f"DEBUG - position_ids shape: {position_ids.shape}")
        # print(f"DEBUG - position_embeddings weight shape: {self.position_embeddings.weight.shape}")
