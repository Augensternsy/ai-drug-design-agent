# ====================================================================================
# 文件作用:
#   实现支持自条件化 (Self-Conditioning) 机制的高斯扩散过程 (GaussianDiffusion)。
#
# 参照原文件:
#   improved_diffusion/gaussian_diffusion.py
#
# 所做的修改与目的:
#   1. 在 p_mean_variance 中引入自条件化嵌入拼接逻辑：
#      - 若模型开启 self_cond，从 model_kwargs 中提取 self_cond_x0，拼接在 x_t 的通道维度作为模型输入。
#   2. 修改 p_sample_loop_progressive 与 ddim_sample_loop_progressive：
#      - 缓存上一个 timestep 预测的 pred_xstart 作为自条件化参考输入传递给下一时间步。
#   3. 修改 training_losses_e2e 损失计算逻辑：
#      - 在训练阶段引入 50% 概率的双前向 pass 计算：不带梯度地先预测一次 pred_xstart，
#        detach 后的结果作为自条件输入拼接到 x_t 参与最终有梯度的 Loss 传递。
# ====================================================================================

"""
This code is based on https://github.com/openai/guided-diffusion (Self-Conditioning Version)
"""
...
def get_raw_model(model):
    curr_model = model
    while hasattr(curr_model, 'model') or hasattr(curr_model, 'module'):
        if hasattr(curr_model, 'model'):
            curr_model = curr_model.model
        elif hasattr(curr_model, 'module'):
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        self_cond = getattr(raw_model, 'self_cond', False)
        if self_cond:
            if model_kwargs is None:
                model_kwargs = {}
            else:
                model_kwargs = model_kwargs.copy()
            self_cond_x0 = model_kwargs.pop('self_cond_x0', None)
            if self_cond_x0 is None:
                self_cond_x0 = th.zeros_like(x)
            x_input = th.cat([x, self_cond_x0], dim=-1)
        else:
            x_input = x
            if model_kwargs is None:
                model_kwargs = {}
        ...
        assert t.shape == (B,)
        # print(x.shape)
        model_output = model(x_input, self._scale_timesteps(t),desc[0],desc[1] ,**model_kwargs)
    ...
    def p_sample_loop_progressive(
        ...
        self_cond = getattr(raw_model, 'self_cond', False)
        if self_cond:
            self_cond_x0 = th.zeros(*shape, device=device)
        else:
            self_cond_x0 = None
        ...
        for i in indices:
            ...
            if self_cond:
                if model_kwargs is None:
                    ...
                    model_kwargs = model_kwargs.copy()
                model_kwargs['self_cond_x0'] = self_cond_x0
            with th.no_grad():
                out = self.p_sample(
                    model,
                    img,
    ...
    def training_losses_e2e(self, model, micro, t, model_kwargs=None, noise=None):
        ...
        # input_ids = x_start
        # x_start_mean = (model.model.module.get_embeds if hasattr(model.model, "module") else model.model.get_embeds)(input_ids)

        input_ids = micro[0]
        desc_state = micro[1]
        # print(f"desc_mask 形状: {desc_state.shape}")
        desc_mask = micro[2]
        corrupt_ids = micro[3]
        # print(f"DEBUG - input_ids shape: {input_ids.shape}")
        # print(f"DEBUG - corrupt_ids shape: {corrupt_ids.shape}")
        assert(corrupt_ids.shape==input_ids.shape)
        # assert((corrupt_ids==input_ids).all()) # only for check


        #########################################
        # 使用 expand 确保条件张量与 input_ids 形状匹配
        cond = (t.reshape(-1,1,1) < 400).expand(input_ids.shape)
        mix_ids = th.where(cond, corrupt_ids, input_ids)
        # print(f"DEBUG - mix_ids shape: {mix_ids.shape}")
        if t.max()>self.maxt:
            self.maxt = t.max()
        ...
        # th.tensor([0]).to('cuda')
        raw_model = get_raw_model(model)
        x_start_mean = raw_model.get_embeds(input_ids)
        mix_start_mean = raw_model.get_embeds(mix_ids)
        # print(f"DEBUG - x_start_mean shape: {x_start_mean.shape}")
        # print(f"DEBUG - mix_start_mean shape: {mix_start_mean.shape}")
        # th.tensor([0]).to('cuda')
        ...
        get_logits = raw_model.get_logits

        # --- SELF-CONDITIONING TRAINING UPGRADE ---
        self_cond = getattr(raw_model, 'self_cond', False)
        if self_cond:
            zero_self_cond = th.zeros_like(x_t)
            # 50% probability to use predicted x0 from a first forward pass
            if th.rand(1).item() < 0.5:
                with th.no_grad():
                    x_t_input_prev = th.cat([x_t, zero_self_cond], dim=-1)
                    model_output_prev = model(x_t_input_prev, self._scale_timesteps(t), desc_state, desc_mask)
                    pred_xstart = self.x0_helper(model_output_prev, x_t, t)['pred_xstart']
                    self_cond_x0 = pred_xstart.detach()
            else:
                self_cond_x0 = zero_self_cond
            
            x_t_input = th.cat([x_t, self_cond_x0], dim=-1)
        else:
            x_t_input = x_t
        # ------------------------------------------
        ...
        if self.loss_type == LossType.E2E_KL:
            ...
            model_output = model(x_t_input, self._scale_timesteps(t),desc_state,desc_mask)
            ...
            # print(f"DEBUG - x_start shape: {x_start.shape}")
            # print(f"DEBUG - x_t shape: {x_t.shape}")
            # print(f"DEBUG - mix_start shape: {mix_start.shape}")
            # print(f"DEBUG - input_ids shape: {input_ids.shape}")
